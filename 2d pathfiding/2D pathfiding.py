...
# Node class and helper functions (as you provided) remain the same
class Node:
...
def a_star_pathfinding_with_waypoints(space_corners, square_size, start, waypoints, end, obstacles, movement_rules, host_size):
    """A* Pathfinding algorithm with multiple waypoints."""
    complete_path = []
    current_position = start

    # Go through each waypoint in the list
    for waypoint in waypoints + [end]:  # Treat end as the final waypoint
        path_segment = a_star_pathfinding(space_corners, square_size, current_position, waypoint, obstacles, movement_rules, host_size)
        
        if not path_segment:
            print(f"No path found to waypoint {waypoint}.")
            return None
        
        complete_path.extend(path_segment[1:])  # Add path to current waypoint, avoid repeating positions
        current_position = waypoint  # Move to the current waypoint
...
waypoints = [(15,20),(15,60)]  # Waypoints along the path
obstacle_spaces = [(10,5,20,10),(10,30,20,50),(10,70,20,80)]  # Multiple obstacle regions
...
path = a_star_pathfinding_with_waypoints(space_corners, square_size, start, waypoints, end, obstacles, movement_rules, host_size)
if path:
    print("Path found:")
    for step in path:
        print(step, end = ",")
...
#(0,0,100,0),(0,0,0,100),(0,100,100,100),(100,0,100,100),